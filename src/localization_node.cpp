                      ...
                      double TZ, double dist_diff_A_S, double dist_diff_B_S)
{
  ...
  while (loop_run == 1)
  {
    ...
    N1 = pow(rA, 2) - pow(rS, 2) - 2 * (TZ - SZ) * (SZ - AZ) - pow((SX - AX), 2) - pow((SY - AY), 2) -
         pow((SZ - AZ), 2) + 2 * SX * (SX - AX) + 2 * SY * (SY - AY);
    N2 = pow(rB, 2) - pow(rS, 2) - 2 * (TZ - SZ) * (SZ - BZ) - pow((SX - BX), 2) - pow((SY - BY), 2) -
         pow((SZ - BZ), 2) + 2 * SX * (SX - BX) + 2 * SY * (SY - BY);
